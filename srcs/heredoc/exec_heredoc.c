...
/*   Updated: 2023/06/18 00:42:26 by serif            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	read_heredoc_line(void)
{
	close(g_data.here_fd[1]);
	g_data.main_here_line = malloc(sizeof(char) * 100);
}
...
void	ft_heredoc_line(void)
{
	...
	if (g_data.count_type->heredoc > 0)
		g_data.heredoc = malloc(sizeof(char *) * (g_data.count_type->heredoc * 2 + 1));
	i = 0;
	while (temp)
	{
